...
public class LogsControllerTests
{
    ...
    public async Task List_WhenPageNotSpecified_MustRequestPageOneWithConfiguredPageSize()
    {
        ...
        SetupPagedResult(page: 1, pageSize: 20);

        // Act: Invokes the method under test with the arranged parameters.
        await controller.List();

        // Assert: Verifies that the action of the method under test behaves as expected.
        _userLogService.Verify(s => s.GetPagedAsync(1, 20), Times.Once);
    }
    ...
    public async Task List_WhenPageSpecified_MustPassPageToService()
    {
        // Arrange: Initializes objects and sets the value of the data that is passed to the method under test.
        var controller = CreateController();
        SetupPagedResult(page: 3, pageSize: 20);

        // Act: Invokes the method under test with the arranged parameters.
        await controller.List(3);

        // Assert: Verifies that the action of the method under test behaves as expected.
        _userLogService.Verify(s => s.GetPagedAsync(3, 20), Times.Once);
    }
    ...
    public async Task List_WhenServiceReturnsPagedLogs_MustMapItemsAndPagingInfoOntoViewModel()
    {
        ...
        _userLogService
            .Setup(s => s.GetPagedAsync(1, 20))
            .ReturnsAsync(new PagedResult<UserLog> { Items = logs, Page = 1, PageSize = 20, TotalCount = 21 });
        ...
        result.Model.Should().BeOfType<LogListViewModel>()
            ...
            {
                Items = [new LogListItemViewModel { UserId = 7, Action = UserLogAction.Created, Timestamp = logs[0].Timestamp }],
                Page = 1,
                TotalPages = 2
            });
    }
    ...
    private LogsController CreateController() => new(_userLogService.Object);
}
