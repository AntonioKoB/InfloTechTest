...
/// <summary>
/// Locks in the Blazor host's telemetry registration: Application Insights is wired only when a connection
/// string is configured, so a local run without one starts exactly as before and sends nothing. What the SDK
/// collects once registered (requests, the HTTP calls to the API, exceptions, logs) is the SDK's contract,
/// proven against the deployed environment rather than here.
/// </summary>
public class TelemetryExtensionsTests
{
    ...
    [Fact]
    public void AddBlazorTelemetry_WhenConnectionStringConfigured_MustRegisterTelemetryClientWithIt()
    {
        // Arrange
