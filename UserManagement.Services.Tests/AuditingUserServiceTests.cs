...
public class AuditingUserServiceTests
{
    ...
    [Fact]
    public async Task GetByIdAsync_WhenCalled_MustNotRecordAnyLog()
    {
        // Arrange: Initializes objects and sets the value of the data that is passed to the method under test.
        // Deliberate design decision: GetByIdAsync is shared by the View screen, Edit's form pre-fill, Edit's
        // own re-fetch before saving, and Delete's confirmation screen - the decorator can't tell which of
        // these a call is for, so it records nothing here. Only the View controller action records "Viewed"
        // explicitly, since it's the one unambiguous case. See UsersController.View.
        var service = CreateService();
        var user = new User { Id = 5, Forename = "Existing", Surname = "User", Email = "existing@example.com", DateOfBirth = new DateOnly(1990, 1, 1) };
        _inner.Setup(s => s.GetByIdAsync(5)).ReturnsAsync(user);
